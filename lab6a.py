...
class Student:
    ...
    def __init__(self, name, number):
        self.name = name
        self.number = str(number)  # Convert student number to string to prevent TypeError
        self.courses = {}

    # Return student name and number
    def displayStudent(self):
        return 'Student Name: ' + self.name + '\nStudent Number: ' + self.number  # Adjusted for output format
    ...
    def displayCourses(self):
        passed_courses = [course for course, grade in self.courses.items() if grade > 0]
